...
if __name__ == '__main__':
    ...
    for file in os.listdir(AUDIO):
        ...
        y, _ = librosa.load(AUDIO + file, offset=OFFSET, duration=DURATION)
        pool.apply_async(create_chromagram, args=(y, audio_name), callback=update)
        pool.apply_async(create_mel_spectrogram, args=(y, audio_name))
        pool.apply_async(create_mfcc, args=(y, audio_name))
        pool.apply_async(create_stft_harmonic, args=(y, audio_name))
        pool.apply_async(create_stft_percussive, args=(y, audio_name))
        pool.apply_async(create_waveform, args=(y, audio_name))
    pool.close()
    pool.join()
