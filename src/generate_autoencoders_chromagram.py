...
dataframe = pd.read_csv(TRAIN_ANNOTATIONS, skiprows=range(1, 2700), nrows=2700)
...
# restored_train_imgs = model.predict_generator(generator=train_generator,
#                                               steps=STEP_SIZE_TRAIN,
#                                               max_queue_size=100)
#
# test_generator.reset()
# restored_test_imgs = model.predict_generator(generator=test_generator,
#                                              steps=STEP_SIZE_TEST,
#                                              max_queue_size=100)
#
# valid_generator.reset()
# restored_valid_imgs = model.predict_generator(generator=valid_generator,
#                                               steps=STEP_SIZE_VALID,
#                                               max_queue_size=100)
#
# print(restored_valid_imgs)
...
#         pylab.savefig(AUTOENCODERS_CHROMAGRAM + audio_name, bbox_inches=None, pad_inches=0, format='png', dpi=100)
#         pylab.close()
#
#
...
def generate_valid_autoencoders():
    for audio_name in valid_generator.filenames:
        ...
        pylab.axis('off')
        pylab.axes([0., 0., 1., 1.], frameon=False, xticks=[], yticks=[])
        pylab.imshow(prediction_generator[valid_generator.filenames.index(audio_name)])
        pylab.savefig(AUTOENCODERS_CHROMAGRAM + audio_name, bbox_inches=None, pad_inches=0, format='png', dpi=100)
        pylab.close()
