...
sys.path.append('src')
from generate_structure import MODEL_AUTOENCODERS, BINARY_ANNOTATIONS, VALIDATION_ANNOTATIONS, AUDIO_MEL_SPECTROGRAM, \
 AUTOENCODERS_MEL_SPECTROGRAM
...
# df_1 = pd.read_csv(BINARY_ANNOTATIONS, nrows=4000)
# df_2 = pd.read_csv(BINARY_ANNOTATIONS, skiprows=range(1, 4000), nrows=4000)
# df_3 = pd.read_csv(BINARY_ANNOTATIONS, skiprows=range(1, 8000), nrows=4000)
...
predict_generator = datagen.flow_from_dataframe(
    dataframe=pd.read_csv(BINARY_ANNOTATIONS, nrows=4000),
...
model = load_model(MODEL_AUTOENCODERS + 'model_mel_spectrogram.h5')
...
def generate_predict_autoencoders():
    for audio_name in predict_generator.filenames:
        ...
        pylab.savefig(AUTOENCODERS_MEL_SPECTROGRAM + audio_name, bbox_inches=None, pad_inches=0, format='png', dpi=100)
        pylab.close()
