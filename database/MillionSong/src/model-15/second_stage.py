...
from model import cnn_cnn_model_15_s2
sys.path.append('src')
...
from generate_structure import TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, AUTOENCODERS_CHROMAGRAM, \
 MODEL_15_TENSOR, MODEL_15_WEIGHTS_FINAL, MODEL_15_OUT_SECOND_STAGE
...
train_generator = datagen.flow_from_dataframe(
    ...
    directory=AUTOENCODERS_CHROMAGRAM,
    x_col='song_name',
    y_col=columns,
...
model = cnn_cnn_model_15_s2()

model.load_weights(MODEL_15_WEIGHTS_FINAL + 'weights_first_stage.h5', by_name=True, skip_mismatch=True)
...
callbacks_list = [
    ...
    TensorBoard(log_dir=MODEL_15_TENSOR + 'second_stage/' + datetime_str, histogram_freq=0, write_graph=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=REDUCE_LR, min_lr=1e-10, mode='auto', verbose=1),
    CSVLogger(MODEL_15_OUT_SECOND_STAGE + 'training.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_15_OUT_SECOND_STAGE + "testing.csv", index=False)
...
results_proba.to_csv(MODEL_15_OUT_SECOND_STAGE + "y_proba_stage_2.csv", index=False)
...
results_pred.to_csv(MODEL_15_OUT_SECOND_STAGE + "y_pred_stage_2.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_15_OUT_SECOND_STAGE, 'model_accuracy_second_stage.png')
    generate_loss_graph(history, MODEL_15_OUT_SECOND_STAGE, 'model_loss_second_stage.png')
    generate_auc_roc_graph(history, MODEL_15_OUT_SECOND_STAGE, 'model_auc_roc_second_stage.png')
    generate_auc_pr_graph(history, MODEL_15_OUT_SECOND_STAGE, 'model_auc_pr_second_stage.png')
    generate_hamming_loss_graph(history, MODEL_15_OUT_SECOND_STAGE, 'model_hamming_loss_second_stage.png')
    generate_ranking_loss_graph(history, MODEL_15_OUT_SECOND_STAGE, 'model_ranking_loss_second_stage.png')
    plot_model(model, to_file=MODEL_15_OUT_SECOND_STAGE + 'cnn_model_second_stage.png')
