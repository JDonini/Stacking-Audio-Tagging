...
model = Sequential()
model.add(VGG16(weights=None, include_top=False, input_tensor=Input(shape=IMG_SIZE)))
model.add(Flatten())
...
callbacks_list = [
    ModelCheckpoint(MODEL_13_WEIGHTS_FINAL + 'weights_first_stage_vgg_16.h5', save_weights_only=True, save_best_only=True),
    ...
    CSVLogger(MODEL_13_OUT_FIRST_STAGE + 'training_vgg_16.csv', append=True, separator=',')
]
...
results.to_csv(MODEL_13_OUT_FIRST_STAGE + "predictions_vgg_16.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_13_OUT_FIRST_STAGE, 'model_accuracy_first_stage_vgg_16.png')
    generate_loss_graph(history, MODEL_13_OUT_FIRST_STAGE, 'model_loss_first_stage_vgg_16.png')
    generate_auc_roc_graph(history, MODEL_13_OUT_FIRST_STAGE, 'model_auc_roc_first_stage_vgg_16.png')
    generate_auc_pr_graph(history, MODEL_13_OUT_FIRST_STAGE, 'model_auc_pr_first_stage_vgg_16.png')
    generate_hamming_loss_graph(history, MODEL_13_OUT_FIRST_STAGE, 'model_hamming_loss_first_stage_vgg_16.png')
    generate_ranking_loss_graph(history, MODEL_13_OUT_FIRST_STAGE, 'model_ranking_loss_first_stage_vgg_16.png')
    plot_model(model, to_file=MODEL_13_OUT_FIRST_STAGE + 'cnn_model_first_stage_vgg_16.png')
