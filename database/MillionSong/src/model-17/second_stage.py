...
from model import cnn_cnn_model_17_s2
sys.path.append('src')
...
model = cnn_cnn_model_17_s2()

model.load_weights(MODEL_17_WEIGHTS_FINAL + 'weights_first_stage.h5', by_name=True, skip_mismatch=True)
...
results_testing.to_csv(MODEL_17_OUT_SECOND_STAGE + "testing.csv", index=False)
...
results_proba.to_csv(MODEL_17_OUT_SECOND_STAGE + "y_proba_stage_2.csv", index=False)
...
results_pred.to_csv(MODEL_17_OUT_SECOND_STAGE + "y_pred_stage_2.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_17_OUT_SECOND_STAGE, 'model_accuracy_second_stage.png')
    generate_loss_graph(history, MODEL_17_OUT_SECOND_STAGE, 'model_loss_second_stage.png')
    generate_auc_roc_graph(history, MODEL_17_OUT_SECOND_STAGE, 'model_auc_roc_second_stage.png')
    generate_auc_pr_graph(history, MODEL_17_OUT_SECOND_STAGE, 'model_auc_pr_second_stage.png')
    generate_hamming_loss_graph(history, MODEL_17_OUT_SECOND_STAGE, 'model_hamming_loss_second_stage.png')
    generate_ranking_loss_graph(history, MODEL_17_OUT_SECOND_STAGE, 'model_ranking_loss_second_stage.png')
    plot_model(model, to_file=MODEL_17_OUT_SECOND_STAGE + 'cnn_model_second_stage.png')
