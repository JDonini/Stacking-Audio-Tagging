...
from model import cnn_cnn_model_16_s2
sys.path.append('src')
...
from generate_structure import TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, AUTOENCODERS_MEL_SPECTROGRAM, \
 MODEL_16_TENSOR, MODEL_16_WEIGHTS_FINAL, MODEL_16_OUT_SECOND_STAGE
...
model = cnn_cnn_model_16_s2()

model.load_weights(MODEL_16_WEIGHTS_FINAL + 'weights_first_stage.h5', by_name=True, skip_mismatch=True)
...
results_proba.to_csv(MODEL_16_OUT_SECOND_STAGE + "y_proba_stage_2.csv", index=False)
...
results_pred.to_csv(MODEL_16_OUT_SECOND_STAGE + "y_pred_stage_2.csv", index=False)
...
if __name__ == '__main__':
    ...
    plot_model(model, to_file=MODEL_16_OUT_SECOND_STAGE + 'cnn_model_second_stage.png')
