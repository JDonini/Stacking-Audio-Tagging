...
callbacks_list = [
    ModelCheckpoint(MODEL_12_WEIGHTS_FINAL + 'weights_first_stage_xception.h5', save_weights_only=True, save_best_only=True),
    ...
    CSVLogger(MODEL_12_OUT_FIRST_STAGE + 'training_xception.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_12_OUT_FIRST_STAGE + "testing_xception.csv", index=False)
...
results.to_csv(MODEL_12_OUT_FIRST_STAGE + "predictions_xception.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_accuracy_first_stage_xception.png')
    generate_loss_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_loss_first_stage_xception.png')
    generate_auc_roc_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_auc_roc_first_stage_xception.png')
    generate_auc_pr_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_auc_pr_first_stage_xception.png')
    generate_hamming_loss_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_hamming_loss_first_stage_xception.png')
    generate_ranking_loss_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_ranking_loss_second_stage_xception.png')
    plot_model(model, to_file=MODEL_12_OUT_FIRST_STAGE + 'cnn_model_first_stage_xception.png')
