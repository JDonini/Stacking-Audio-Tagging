...
callbacks_list = [
    ModelCheckpoint(MODEL_12_WEIGHTS_FINAL + 'weights_first_stage_vgg_19.h5', save_weights_only=True, save_best_only=True),
   EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=EARLY_STOPPING),
    ...
    CSVLogger(MODEL_12_OUT_FIRST_STAGE + 'training_vgg_19.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_12_OUT_FIRST_STAGE + "testing_vgg_19.csv", index=False)
...
results.to_csv(MODEL_12_OUT_FIRST_STAGE + "predictions_vgg_19.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_accuracy_first_stage_vgg_19.png')
    generate_loss_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_loss_first_stage_vgg_19.png')
    generate_auc_roc_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_auc_roc_first_stage_vgg_19.png')
    generate_auc_pr_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_auc_pr_first_stage_vgg_19.png')
    generate_hamming_loss_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_hamming_loss_first_stage_vgg_19.png')
    generate_ranking_loss_graph(history, MODEL_12_OUT_FIRST_STAGE, 'model_ranking_loss_second_stage_vgg_19.png')
    plot_model(model, to_file=MODEL_12_OUT_FIRST_STAGE + 'cnn_model_first_stage_vgg_19.png')
