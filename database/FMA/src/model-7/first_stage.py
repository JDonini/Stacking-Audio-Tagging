...
from model import cnn_cnn_model_7
sys.path.append('src/')
...
callbacks_list = [
    ModelCheckpoint(MODEL_7_WEIGHTS_FINAL + 'weights_first_stage.h5', save_weights_only=True, save_best_only=True),
    ...
    CSVLogger(MODEL_7_OUT_FIRST_STAGE + 'training.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_7_OUT_FIRST_STAGE + "testing.csv", index=False)
...
results_proba.to_csv(MODEL_7_OUT_FIRST_STAGE + "y_proba_stage_1.csv", index=False)
...
results_pred.to_csv(MODEL_7_OUT_FIRST_STAGE + "y_pred_stage_1.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_7_OUT_FIRST_STAGE, 'model_accuracy_first_stage.png')
    generate_loss_graph(history, MODEL_7_OUT_FIRST_STAGE, 'model_loss_first_stage.png')
    generate_auc_roc_graph(history, MODEL_7_OUT_FIRST_STAGE, 'model_auc_roc_first_stage.png')
    generate_auc_pr_graph(history, MODEL_7_OUT_FIRST_STAGE, 'model_auc_pr_first_stage.png')
    generate_hamming_loss_graph(history, MODEL_7_OUT_FIRST_STAGE, 'model_hamming_loss_first_stage.png')
    generate_ranking_loss_graph(history, MODEL_7_OUT_FIRST_STAGE, 'model_ranking_loss_first_stage.png')
    plot_model(model, to_file=MODEL_7_OUT_FIRST_STAGE + 'cnn_model_first_stage.png')
