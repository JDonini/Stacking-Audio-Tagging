...
from generate_structure import TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, AUTOENCODERS_MEL_SPECTROGRAM, \
 MODEL_13_TENSOR, MODEL_13_WEIGHTS_FINAL, MODEL_13_WEIGTHS_PER_EPOCHS, MODEL_13_OUT_SECOND_STAGE
...
callbacks_list = [
    ModelCheckpoint(MODEL_13_WEIGTHS_PER_EPOCHS + 'weights_second_stage_{epoch:03d}.h5', save_weights_only=True, save_best_only=True),
    ...
    CSVLogger(MODEL_13_OUT_SECOND_STAGE + 'training.csv', append=True, separator=',')
]
...
results.to_csv(MODEL_13_OUT_SECOND_STAGE + "predictions.csv", index=False)


if __name__ == '__main__':
    K.clear_session()
    generate_acc_graph(history, MODEL_13_OUT_SECOND_STAGE, 'model_accuracy_second_stage.png')
    generate_loss_graph(history, MODEL_13_OUT_SECOND_STAGE, 'model_loss_second_stage.png')
    generate_auc_roc_graph(history, MODEL_13_OUT_SECOND_STAGE, 'model_auc_roc_second_stage.png')
    generate_auc_pr_graph(history, MODEL_13_OUT_SECOND_STAGE, 'model_auc_pr_second_stage.png')
    generate_hamming_loss_graph(history, MODEL_13_OUT_SECOND_STAGE, 'model_hamming_loss_second_stage.png')
    generate_ranking_loss_graph(history, MODEL_13_OUT_SECOND_STAGE, 'model_ranking_loss_second_stage.png')
    plot_model(model, to_file=MODEL_13_OUT_SECOND_STAGE + 'cnn_model_13_second_stage.png')
