...
from generate_structure import AUDIO_STFT_HARMONIC, AUDIO_STFT_PERCUSSIVE, TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, \
 MODEL_3_TENSOR, MODEL_3_WEIGHTS_FINAL, MODEL_3_WEIGTHS_PER_EPOCHS, MODEL_3_OUT_SECOND_STAGE
...
if len(K.tensorflow_backend._get_available_gpus()) > 1:
    model = multi_gpu_model(merge_model_3(), gpus=len(K.tensorflow_backend._get_available_gpus()))
else:
    model = merge_model_3()

model.load_weights(MODEL_3_WEIGHTS_FINAL + 'first_stage.h5')
...
callbacks_list = [
    ModelCheckpoint(MODEL_3_WEIGTHS_PER_EPOCHS + 'weights_second_stage_{epoch:03d}.h5', save_weights_only=True, save_best_only=True),
...
results.to_csv(MODEL_3_OUT_SECOND_STAGE + "predictions.csv", index=False)
...
if __name__ == '__main__':
    ...
    plot_model(model, to_file=MODEL_3_OUT_SECOND_STAGE + 'cnn_model_3_second_stage.png')
