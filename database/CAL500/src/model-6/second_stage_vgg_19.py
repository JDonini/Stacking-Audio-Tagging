...
if len(k.tensorflow_backend._get_available_gpus()) > 1:
    model = multi_gpu_model(VGG19(include_top=False, pooling='max', weights=None, input_shape=IMG_SIZE),
                            gpus=len(k.tensorflow_backend._get_available_gpus()))
else:
    model = VGG19(include_top=False, pooling='max', weights=None, input_shape=IMG_SIZE)

model.load_weights(MODEL_6_WEIGHTS_FINAL + 'weights_first_stage_vgg_19.h5')
...
callbacks_list = [
    ...
    CSVLogger(MODEL_6_OUT_SECOND_STAGE + 'training_vgg_19.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_6_OUT_SECOND_STAGE + "testing_vgg_19.csv", index=False)
...
results.to_csv(MODEL_6_OUT_SECOND_STAGE + "predictions_vgg_19.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_6_OUT_SECOND_STAGE, 'model_accuracy_second_stage_vgg_19.png')
    generate_loss_graph(history, MODEL_6_OUT_SECOND_STAGE, 'model_loss_second_stage_vgg_19.png')
    generate_auc_roc_graph(history, MODEL_6_OUT_SECOND_STAGE, 'model_auc_roc_second_stage_vgg_19.png')
    generate_auc_pr_graph(history, MODEL_6_OUT_SECOND_STAGE, 'model_auc_pr_second_stage_vgg_19.png')
    generate_hamming_loss_graph(history, MODEL_6_OUT_SECOND_STAGE, 'model_hamming_loss_second_stage_vgg_19.png')
    generate_ranking_loss_graph(history, MODEL_6_OUT_SECOND_STAGE, 'model_ranking_loss_second_stage_vgg_19.png')
    plot_model(model, to_file=MODEL_6_OUT_SECOND_STAGE + 'cnn_model_second_stage_vgg_19.png')
