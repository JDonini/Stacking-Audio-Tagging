...
from model import merge_model_15
sys.path.append('src')
...
from generate_structure import AUDIO_MEL_SPECTROGRAM, AUTOENCODERS_MEL_SPECTROGRAM, TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, \
 MODEL_15_TENSOR, MODEL_15_WEIGHTS_FINAL, MODEL_15_WEIGTHS_PER_EPOCHS, MODEL_15_OUT_FIRST_STAGE
...
if len(K.tensorflow_backend._get_available_gpus()) > 1:
    model = multi_gpu_model(merge_model_15(), gpus=len(K.tensorflow_backend._get_available_gpus()))
else:
    model = merge_model_15()
...
callbacks_list = [
    ModelCheckpoint(MODEL_15_WEIGTHS_PER_EPOCHS + 'weights_first_stage_{epoch:03d}.h5', save_weights_only=True, save_best_only=True),
    EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20),
    EarlyStopping(monitor='val_acc', mode='max', patience=20),
    TensorBoard(log_dir=MODEL_15_TENSOR + 'first_stage/' + datetime_str, histogram_freq=0, write_graph=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=12, min_lr=1e-10, mode='auto', verbose=1),
    CSVLogger(MODEL_15_OUT_FIRST_STAGE + 'training.csv', append=True, separator=',')
]
...
score = model.evaluate_generator(validation_generate_multiple_input(
), steps=STEP_SIZE_VALID, verbose=0, max_queue_size=100)
...
results_testing.to_csv(MODEL_15_OUT_FIRST_STAGE + "testing.csv", index=False)
...
results.to_csv(MODEL_15_OUT_FIRST_STAGE + "predictions.csv", index=False)

if __name__ == '__main__':
    K.clear_session()
    generate_acc_graph(history, MODEL_15_OUT_FIRST_STAGE, 'model_accuracy_first_stage.png')
    generate_loss_graph(history, MODEL_15_OUT_FIRST_STAGE, 'model_loss_first_stage.png')
    generate_auc_roc_graph(history, MODEL_15_OUT_FIRST_STAGE, 'model_auc_roc_first_stage.png')
    generate_auc_pr_graph(history, MODEL_15_OUT_FIRST_STAGE, 'model_auc_pr_first_stage.png')
    generate_hamming_loss_graph(history, MODEL_15_OUT_FIRST_STAGE, 'model_hamming_loss_first_stage.png')
    generate_ranking_loss_graph(history, MODEL_15_OUT_FIRST_STAGE, 'model_ranking_loss_first_stage.png')
    plot_model(model, to_file=MODEL_15_OUT_FIRST_STAGE + 'cnn_model_15_first_stage.png')
