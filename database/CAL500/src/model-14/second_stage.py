...
from model import cnn_cnn_model_14
sys.path.append('src')
...
from generate_graph import generate_acc_graph, generate_loss_graph, generate_auc_roc_graph, generate_auc_pr_graph, \
 generate_hamming_loss_graph, generate_ranking_loss_graph, accuracy
from generate_structure import TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, AUTOENCODERS_MEL_SPECTROGRAM, \
 MODEL_14_TENSOR, MODEL_14_WEIGHTS_FINAL, MODEL_14_WEIGTHS_PER_EPOCHS, MODEL_14_OUT_SECOND_STAGE
...
if len(K.tensorflow_backend._get_available_gpus()) > 1:
    model = multi_gpu_model(cnn_cnn_model_14(), gpus=len(K.tensorflow_backend._get_available_gpus()))
else:
    model = cnn_cnn_model_14()

model.load_weights(MODEL_14_WEIGHTS_FINAL + 'first_stage.h5')
...
callbacks_list = [
    ModelCheckpoint(MODEL_14_WEIGTHS_PER_EPOCHS + 'weights_second_stage_{epoch:03d}.h5', save_weights_only=True, save_best_only=True),
    EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20),
    EarlyStopping(monitor='val_acc', mode='max', patience=20),
    TensorBoard(log_dir=MODEL_14_TENSOR + 'second_stage/' + datetime_str, histogram_freq=0, write_graph=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=12, min_lr=1e-10, mode='auto', verbose=1),
    CSVLogger(MODEL_14_OUT_SECOND_STAGE + 'training.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_14_OUT_SECOND_STAGE + "testing.csv", index=False)
...
results.to_csv(MODEL_14_OUT_SECOND_STAGE + "predictions.csv", index=False)


if __name__ == '__main__':
    K.clear_session()
    generate_acc_graph(history, MODEL_14_OUT_SECOND_STAGE, 'model_accuracy_second_stage.png')
    generate_loss_graph(history, MODEL_14_OUT_SECOND_STAGE, 'model_loss_second_stage.png')
    generate_auc_roc_graph(history, MODEL_14_OUT_SECOND_STAGE, 'model_auc_roc_second_stage.png')
    generate_auc_pr_graph(history, MODEL_14_OUT_SECOND_STAGE, 'model_auc_pr_second_stage.png')
    generate_hamming_loss_graph(history, MODEL_14_OUT_SECOND_STAGE, 'model_hamming_loss_second_stage.png')
    generate_ranking_loss_graph(history, MODEL_14_OUT_SECOND_STAGE, 'model_ranking_loss_second_stage.png')
    plot_model(model, to_file=MODEL_14_OUT_SECOND_STAGE + 'cnn_model_14_second_stage.png')
