...
from model import cnn_cnn_model_10_s2
sys.path.append('src')
...
if len(k.tensorflow_backend._get_available_gpus()) > 1:
    model = multi_gpu_model(cnn_cnn_model_10_s2(), gpus=len(k.tensorflow_backend._get_available_gpus()))
else:
    model = cnn_cnn_model_10_s2()

model.compile(loss='binary_crossentropy', optimizer=RMSprop(
    lr=LR, decay=LR_DECAY), metrics=['accuracy', auc_roc, auc_pr, hamming_loss, ranking_loss])

model.load_weights(MODEL_10_WEIGHTS_FINAL + 'weights_first_stage.h5', by_name=True, skip_mismatch=True)
...
if __name__ == '__main__':
    ...
    plot_model(model, to_file=MODEL_10_OUT_SECOND_STAGE + 'cnn_model_14_second_stage.png')
