...
from generate_structure import AUDIO_STFT, TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, \
 MODEL_8_TENSOR, MODEL_8_WEIGHTS_FINAL, MODEL_8_OUT_FIRST_STAGE
...
callbacks_list = [
    ModelCheckpoint(MODEL_8_WEIGHTS_FINAL + 'weights_first_stage_inception_resnet_v2.h5', save_weights_only=True, save_best_only=True),
    EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20),
    EarlyStopping(monitor='val_acc', mode='max', patience=20),
    TensorBoard(log_dir=MODEL_8_TENSOR + 'first_stage/' + datetime_str, histogram_freq=0, write_graph=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=12, min_lr=1e-10, mode='auto', verbose=1),
    CSVLogger(MODEL_8_OUT_FIRST_STAGE + 'training_inception_resnet_v2.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_8_OUT_FIRST_STAGE + "testing_inception_resnet_v2.csv", index=False)
...
results.to_csv(MODEL_8_OUT_FIRST_STAGE + "predictions_inception_resnet_v2.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_8_OUT_FIRST_STAGE, 'model_accuracy_first_stage_inception_resnet_v2.png')
    generate_loss_graph(history, MODEL_8_OUT_FIRST_STAGE, 'model_loss_first_stage_inception_resnet_v2.png')
    generate_auc_roc_graph(history, MODEL_8_OUT_FIRST_STAGE, 'model_auc_roc_first_stage_inception_resnet_v2.png')
    generate_auc_pr_graph(history, MODEL_8_OUT_FIRST_STAGE, 'model_auc_pr_first_stage_inception_resnet_v2.png')
    generate_hamming_loss_graph(history, MODEL_8_OUT_FIRST_STAGE, 'model_hamming_loss_first_stage_inception_resnet_v2.png')
    generate_ranking_loss_graph(history, MODEL_8_OUT_FIRST_STAGE, 'model_ranking_loss_second_stage_inception_resnet_v2.png')
    plot_model(model, to_file=MODEL_8_OUT_FIRST_STAGE + 'cnn_model_first_stage_inception_resnet_v2.png')
