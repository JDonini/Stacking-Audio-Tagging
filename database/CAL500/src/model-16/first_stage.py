...
callbacks_list = [
    ModelCheckpoint(MODEL_16_WEIGTHS_PER_EPOCHS + 'weights_first_stage_{epoch:03d}.h5', save_weights_only=True, save_best_only=True),
    EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20),
    EarlyStopping(monitor='val_acc', mode='max', patience=20),
    TensorBoard(log_dir=MODEL_16_TENSOR + 'first_stage/' + datetime_str, histogram_freq=0, write_graph=True),
...
model.save_weights(MODEL_16_WEIGHTS_FINAL + 'first_stage.h5')
...
results.to_csv(MODEL_16_OUT_FIRST_STAGE + "predictions.csv", index=False)
...
if __name__ == '__main__':
    ...
    plot_model(model, to_file=MODEL_16_OUT_FIRST_STAGE + 'cnn_model_16_first_stage.png')
