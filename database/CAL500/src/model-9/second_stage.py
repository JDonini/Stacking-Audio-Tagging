...
from model import cnn_cnn_model_9_s2
sys.path.append('src')
...
from generate_structure import TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, AUTOENCODERS_MEL_SPECTROGRAM, \
 MODEL_9_TENSOR, MODEL_9_WEIGHTS_FINAL, MODEL_9_OUT_SECOND_STAGE
...
if len(k.tensorflow_backend._get_available_gpus()) > 1:
    model = multi_gpu_model(cnn_cnn_model_9_s2(), gpus=len(k.tensorflow_backend._get_available_gpus()))
else:
    model = cnn_cnn_model_9_s2()
...
callbacks_list = [
    ...
    TensorBoard(log_dir=MODEL_9_TENSOR + 'second_stage/' + datetime_str, histogram_freq=0, write_graph=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=12, min_lr=1e-10, mode='auto', verbose=1),
    CSVLogger(MODEL_9_OUT_SECOND_STAGE + 'training.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_9_OUT_SECOND_STAGE + "testing.csv", index=False)
...
results.to_csv(MODEL_9_OUT_SECOND_STAGE + "predictions.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_9_OUT_SECOND_STAGE, 'model_accuracy_second_stage.png')
    generate_loss_graph(history, MODEL_9_OUT_SECOND_STAGE, 'model_loss_second_stage.png')
    generate_auc_roc_graph(history, MODEL_9_OUT_SECOND_STAGE, 'model_auc_roc_second_stage.png')
    generate_auc_pr_graph(history, MODEL_9_OUT_SECOND_STAGE, 'model_auc_pr_second_stage.png')
    generate_hamming_loss_graph(history, MODEL_9_OUT_SECOND_STAGE, 'model_hamming_loss_second_stage.png')
    generate_ranking_loss_graph(history, MODEL_9_OUT_SECOND_STAGE, 'model_ranking_loss_second_stage.png')
    plot_model(model, to_file=MODEL_9_OUT_SECOND_STAGE + 'cnn_model_13_second_stage.png')
