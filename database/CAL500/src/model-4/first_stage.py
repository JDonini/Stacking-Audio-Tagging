...
from generate_structure import AUDIO_MFCC, AUDIO_MEL_SPECTROGRAM, TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, \
 MODEL_4_TENSOR, MODEL_4_WEIGHTS_FINAL, MODEL_4_WEIGTHS_PER_EPOCHS, MODEL_4_OUT_FIRST_STAGE
...
callbacks_list = [
    ModelCheckpoint(MODEL_4_WEIGTHS_PER_EPOCHS + 'weights_first_stage_{epoch:03d}.h5', save_weights_only=True, save_best_only=True),
    EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20),
    EarlyStopping(monitor='val_acc', mode='max', patience=20),
    TensorBoard(log_dir=MODEL_4_TENSOR + 'first_stage/' + datetime_str, histogram_freq=0, write_graph=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=12, min_lr=1e-10, mode='auto', verbose=1),
    CSVLogger(MODEL_4_OUT_FIRST_STAGE + 'training.csv', append=True, separator=',')
]

model.save_weights(MODEL_4_WEIGHTS_FINAL + 'first_stage.h5')
...
results_testing.to_csv(MODEL_4_OUT_FIRST_STAGE + "testing.csv", index=False)
...
results.to_csv(MODEL_4_OUT_FIRST_STAGE + "predictions.csv", index=False)

if __name__ == '__main__':
    K.clear_session()
    generate_acc_graph(history, MODEL_4_OUT_FIRST_STAGE, 'model_accuracy_first_stage.png')
    generate_loss_graph(history, MODEL_4_OUT_FIRST_STAGE, 'model_loss_first_stage.png')
    generate_auc_roc_graph(history, MODEL_4_OUT_FIRST_STAGE, 'model_auc_roc_first_stage.png')
    generate_auc_pr_graph(history, MODEL_4_OUT_FIRST_STAGE, 'model_auc_pr_first_stage.png')
    generate_hamming_loss_graph(history, MODEL_4_OUT_FIRST_STAGE, 'model_hamming_loss_first_stage.png')
    generate_ranking_loss_graph(history, MODEL_4_OUT_FIRST_STAGE, 'model_ranking_loss_second_stage.png')
    plot_model(model, to_file=MODEL_4_OUT_FIRST_STAGE + 'cnn_model_4_first_stage.png')
