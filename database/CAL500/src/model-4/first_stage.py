...
try:
    model = multi_gpu_model(cnn_cnn_model_4())
    print('Using GPUs')
except:
...
callbacks_list = [
    ModelCheckpoint(MODEL_4_WEIGHTS_FINAL + 'weights_first_stage.h5', save_weights_only=True, save_best_only=True),
    EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20),
    EarlyStopping(monitor='val_acc', mode='max', patience=20),
    TensorBoard(log_dir=MODEL_4_TENSOR + 'first_stage/' + datetime_str, histogram_freq=0, write_graph=False, write_images=True),
...
if __name__ == '__main__':
    ...
    generate_ranking_loss_graph(history, MODEL_4_OUT_FIRST_STAGE, 'model_ranking_loss_first_stage.png')
    plot_model(model, to_file=MODEL_4_OUT_FIRST_STAGE + 'cnn_model_first_stage.png')
