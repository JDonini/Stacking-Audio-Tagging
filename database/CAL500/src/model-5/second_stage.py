...
from model import merge_late_fusion_model_5_stage_2
sys.path.append('src')
...
if len(k.tensorflow_backend._get_available_gpus()) > 1:
    model = multi_gpu_model(merge_late_fusion_model_5_stage_2(), gpus=len(k.tensorflow_backend._get_available_gpus()))
else:
    model = merge_late_fusion_model_5_stage_2()
...
results_testing.to_csv(MODEL_5_OUT_SECOND_STAGE + "testing.csv", index=False)
...
results.to_csv(MODEL_5_OUT_SECOND_STAGE + "predictions.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_accuracy_second_stage.png')
    generate_loss_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_loss_second_stage.png')
    generate_auc_roc_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_auc_roc_second_stage.png')
    generate_auc_pr_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_auc_pr_second_stage.png')
    generate_hamming_loss_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_hamming_loss_second_stage.png')
    generate_ranking_loss_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_ranking_loss_second_stage.png')
    plot_model(model, to_file=MODEL_5_OUT_SECOND_STAGE + 'cnn_model_second_stage.png')
