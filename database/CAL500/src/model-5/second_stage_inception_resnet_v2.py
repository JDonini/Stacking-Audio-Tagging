...
model.load_weights(MODEL_5_WEIGHTS_FINAL + 'weights_first_stage_inception_resnet_v2.h5')
...
callbacks_list = [
    ...
    TensorBoard(log_dir=MODEL_5_TENSOR + 'second_stage/' + datetime_str, histogram_freq=0, write_graph=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=12, min_lr=1e-10, mode='auto', verbose=1),
    CSVLogger(MODEL_5_OUT_SECOND_STAGE + 'training_inception_resnet_v2.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_5_OUT_SECOND_STAGE + "testing_inception_resnet_v2.csv", index=False)
...
results.to_csv(MODEL_5_OUT_SECOND_STAGE + "predictions_inception_resnet_v2.csv", index=False)


if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_accuracy_second_stage_inception_resnet_v2.png')
    generate_loss_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_loss_second_stage_inception_resnet_v2.png')
    generate_auc_roc_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_auc_roc_second_stage_inception_resnet_v2.png')
    generate_auc_pr_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_auc_pr_second_stage_inception_resnet_v2.png')
    generate_hamming_loss_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_hamming_loss_second_stage_inception_resnet_v2.png')
    generate_ranking_loss_graph(history, MODEL_5_OUT_SECOND_STAGE, 'model_ranking_loss_second_stage_inception_resnet_v2.png')
    plot_model(model, to_file=MODEL_5_OUT_SECOND_STAGE + 'cnn_model_second_stage_inception_resnet_v2.png')
