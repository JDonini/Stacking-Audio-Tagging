...
from model import merge_model_11
sys.path.append('src')
...
from generate_structure import AUTOENCODERS_MEL_SPECTROGRAM, AUDIO_MEL_SPECTROGRAM, TRAIN_ANNOTATIONS, TEST_ANNOTATIONS, VALIDATION_ANNOTATIONS, \
 MODEL_11_TENSOR, MODEL_11_OUT_FIRST_STAGE
...
if len(k.tensorflow_backend._get_available_gpus()) > 1:
    model = multi_gpu_model(merge_model_11(), gpus=len(k.tensorflow_backend._get_available_gpus()))
else:
    model = merge_model_11()
...
callbacks_list = [
    ...
    TensorBoard(log_dir=MODEL_11_TENSOR + 'first_stage/' + datetime_str, histogram_freq=0, write_graph=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=12, min_lr=1e-10, mode='auto', verbose=1),
    CSVLogger(MODEL_11_OUT_FIRST_STAGE + 'training.csv', append=True, separator=',')
]
...
results_testing.to_csv(MODEL_11_OUT_FIRST_STAGE + "testing.csv", index=False)
...
results.to_csv(MODEL_11_OUT_FIRST_STAGE + "predictions.csv", index=False)

if __name__ == '__main__':
    k.clear_session()
    generate_acc_graph(history, MODEL_11_OUT_FIRST_STAGE, 'model_accuracy_first_stage.png')
    generate_loss_graph(history, MODEL_11_OUT_FIRST_STAGE, 'model_loss_first_stage.png')
    generate_auc_roc_graph(history, MODEL_11_OUT_FIRST_STAGE, 'model_auc_roc_first_stage.png')
    generate_auc_pr_graph(history, MODEL_11_OUT_FIRST_STAGE, 'model_auc_pr_first_stage.png')
    generate_hamming_loss_graph(history, MODEL_11_OUT_FIRST_STAGE, 'model_hamming_loss_first_stage.png')
    generate_ranking_loss_graph(history, MODEL_11_OUT_FIRST_STAGE, 'model_ranking_loss_second_stage.png')
    plot_model(model, to_file=MODEL_11_OUT_FIRST_STAGE + 'cnn_model_first_stage.png')
